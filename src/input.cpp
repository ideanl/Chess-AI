...
// Parse FEN notation and setup the board using it.
void Input::parseFen(std::string fen) {
  ...
  for (int i = 0; index >= 0; i++) {
    ...
    while (index & 0x08) { 
      ChessDriver::board.spaces[index].piece = NULL;
      if (index % 16 == 15)
        index -= 31;
      else
        index++;
    ...
    ChessDriver::board.pieces[piece->color][pieceIndex] = piece;

    ChessDriver::board.spaces[index].piece = piece;
